...
// 获取考评列表
global $pdo;
$stmt = $pdo->prepare("SELECT * FROM assessments $where_clause ORDER BY id DESC LIMIT ? OFFSET ?");
$params[] = $limit;
$params[] = $offset;
$stmt->execute($params);
        ...
        <div class="card-body">
            ...
            <?php if ($total_pages > 1): ?>
                <div class="pagination">
                    <a href="?page=1&search=<?php echo $search; ?>&status=<?php echo $status; ?>" class="pagination-btn" <?php echo $page === 1 ? 'disabled' : ''; ?>>首页</a>
                    <a href="?page=<?php echo $page - 1; ?>&search=<?php echo $search; ?>&status=<?php echo $status; ?>" class="pagination-btn" <?php echo $page === 1 ? 'disabled' : ''; ?>>上一页</a>
                    
                    <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                        <a href="?page=<?php echo $i; ?>&search=<?php echo $search; ?>&status=<?php echo $status; ?>" class="pagination-btn <?php echo $i === $page ? 'active' : ''; ?>>><?php echo $i; ?></a>
                    <?php endfor; ?>
                    
                    <a href="?page=<?php echo $page + 1; ?>&search=<?php echo $search; ?>&status=<?php echo $status; ?>" class="pagination-btn" <?php echo $page === $total_pages ? 'disabled' : ''; ?>>下一页</a>
                    <a href="?page=<?php echo $total_pages; ?>&search=<?php echo $search; ?>&status=<?php echo $status; ?>" class="pagination-btn" <?php echo $page === $total_pages ? 'disabled' : ''; ?>>末页</a>
                </div>
            <?php endif; ?>
        </div>
    </div>
