...
$stmt->execute($params); // $params 此时只包含 WHERE 条件参数
...
// 获取考评项目列表
$stmt = $pdo->prepare("SELECT * FROM assessment_items $where_clause ORDER BY $sort $order LIMIT ? OFFSET ?");
// 重建查询参数
    ...
    <div class="card-body">
        <div class="table-container">
            <table class="sortable-table">
                <thead>
                    <tr>
                        <th>
                            <a href="?search=<?php echo $search; ?>&sort=id&order=<?php echo $order === 'ASC' ? 'DESC' : 'ASC'; ?>" class="sort-link <?php echo $sort === 'id' ? 'active' : ''; ?>">
                                ID
        ...
        <?php if ($total_pages > 1): ?>
            <div class="pagination">
                <a href="<?php echo $page > 1 ? "?page=1&search=$search&sort=$sort&order=$order" : 'javascript:void(0);'; ?>" class="pagination-btn" <?php echo $page === 1 ? 'disabled' : ''; ?>>首页</a>
                <a href="<?php echo $page > 1 ? "?page=" . ($page - 1) . "&search=$search&sort=$sort&order=$order" : 'javascript:void(0);'; ?>" class="pagination-btn" <?php echo $page === 1 ? 'disabled' : ''; ?>>上一页</a>

                <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                    <a href="?page=<?php echo $i; ?>&search=<?php echo $search; ?>&sort=<?php echo $sort; ?>&order=<?php echo $order; ?>" class="pagination-btn <?php echo $i === $page ? 'active' : ''; ?>"><?php echo $i; ?></a>
                <?php endfor; ?>

                <a href="<?php echo $page < $total_pages ? "?page=" . ($page + 1) . "&search=$search&sort=$sort&order=$order" : 'javascript:void(0);'; ?>" class="pagination-btn" <?php echo $page === $total_pages ? 'disabled' : ''; ?>>下一页</a>
                <a href="<?php echo $page < $total_pages ? "?page=$total_pages&search=$search&sort=$sort&order=$order" : 'javascript:void(0);'; ?>" class="pagination-btn" <?php echo $page === $total_pages ? 'disabled' : ''; ?>>末页</a>
            </div>
        <?php endif; ?>
    </div>
</div>
